...
class WidgetService {
  ...
  static Future<void> update(String userId) async {
    ...
    try {
      ...
      final hasData = expenses > 0;
      final dateStr = DateFormat('d MMM', 'es').format(now);
      ...
      if (categoryTotals.isNotEmpty) {
        ...
        topCat =
            'Mayor gasto: ${_categoryLabel(top.key)} (${CurrencyFormatter.format(top.value)})';
      }

      await Future.wait([
        HomeWidget.saveWidgetData(
            'widget_expenses', CurrencyFormatter.format(expenses)),
        HomeWidget.saveWidgetData('widget_date', dateStr),
        HomeWidget.saveWidgetData('widget_top_category', topCat),
        HomeWidget.saveWidgetData('widget_has_data', hasData ? '1' : '0'),
      ]);
